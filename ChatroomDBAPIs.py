...
class ChatroomDB:
    def __init__(self):
        ...
        self.cursor.execute("CREATE TABLE IF NOT EXISTS chatroomlist (\
                                ...
                                CHAT_MODEL VARCHAR NOT NULL,\
                                CHAT_PROVIDER VARCHAR NOT NULL,\
    ...
    def createChatroom(self, chatroom_uuid, chatroom_name, embedding_model, chat_model, chat_provider, embedding_provider, user_uuid):
        self.cursor.execute("INSERT INTO chatroomlist (CHATROOM, NAME, EMBEDDING_MODEL, CHAT_MODEL, CHAT_PROVIDER, EMBEDDING_PROVIDER) VALUES(%s, %s, %s, %s, %s, %s)", (str(chatroom_uuid),str(chatroom_name), str(embedding_model), str(chat_model), str(chat_provider), str(embedding_provider)))
    ...
    def getChatProvider(self, chatroom_uuid):
        self.cursor.execute("SELECT CHAT_PROVIDER FROM chatroomlist WHERE CHATROOM = %s", (str(chatroom_uuid),))
        return self.cursor.fetchone()[0]
    
    def getEmbeddingProvider(self, chatroom_uuid):
        self.cursor.execute("SELECT EMBEDDING_PROVIDER FROM chatroomlist WHERE CHATROOM = %s", (str(chatroom_uuid),))
        return self.cursor.fetchone()[0]
    ...
    def getChatrooms(self, user_uuid):
        self.cursor.execute("SELECT list.CHATROOM, list.NAME, list.CHAT_MODEL, list.EMBEDDING_MODEL, list.CHAT_PROVIDER, list.EMBEDDING_PROVIDER FROM chatroomlist list INNER JOIN chatroomroles roles ON list.CHATROOM = roles.CHATROOM WHERE USERUUID = %s", (str(user_uuid),))
        userroomslist = self.cursor.fetchall()
        return userroomslist
    
    def getMessages(self, chatroom_uuid):
        self.cursor.execute("SELECT USERUUID, TIME, MSGUUID, MESSAGE, REFERENCE FROM chatroommessages WHERE CHATROOM = %s", (str(chatroom_uuid),))
    ...
    def updateLLM(self, chatroom_uuid, chatroom_model, model_provider):
        self.cursor.execute("UPDATE chatroomlist SET CHAT_MODEL = %s, CHAT_PROVIDER = %s WHERE CHATROOM = %s", (str(chatroom_model), str(model_provider), str(chatroom_uuid)))
        self.con.commit()
        return